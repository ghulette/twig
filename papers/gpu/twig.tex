...
and define $\mathtt{in}(x+y) = \mathtt{in}(x)$ and $\mathtt{out}(x+y) =
\mathtt{out}(y)$.
...
sequential and parallel composition. Intuitively, $e$ corresponds to a
``no-op,'' and it behaves thus:
...
operator has several possible semantics; we present one interpretation in
...
\[
\infer{\mathtt{tuple}(\ldots,t_i,\ldots) \arr{\#i} (t_i,e)}{}
\]
...
(\mathtt{tuple}(\ldots,t_i',\ldots),m)}
{t_i \arr{s} (t_i',m)}
\]
\[
...
Our implementation of Twig is written in Haskell.
